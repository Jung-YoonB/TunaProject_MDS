...
| 섹션 | 항목 수 | 비고 |
|---|---|---|
| 버그 (기능이 실제로 깨져 있음) | 60개 — 조치 완료 60 · 미해결 0 | **2026-09-03: 전면 재점검 6라운드.** 1라운드(1~21번): review 이미지 공유·빈 IN() 500·상품 STATUS 필터·회원가입 폼·addAddress 신설·`cart.xml` LEFT JOIN 무력화 등. 2라운드(22~28번): 홈 배너 미노출·페이지네이션 5곳 스크롤 복귀·헤더 뱃지 목업·찜 추가 전 화면 서버 미반영·배송지 추가 진입 불가·검색/찜 별점 CSS 누락. 3라운드(29~33번): **옵션 가격이 (실제가-대표가)로 계산되어 기본 옵션이 항상 "0원"**(가장 심각), 카테고리·이동경로·짧은 설명 고정 텍스트, 등급 할인 고정값, 대표이미지 중복 시 500. 4라운드(34~40번): 리뷰 링크 `href="#"`, 장바구니·찜 선택모드 전환+페이지네이션, **장바구니 개별삭제·수량변경 버튼 미반영**, 헤더 뱃지 즉시반영 누락, **로그아웃 후 재로그인 시 찜 상태 미반영**, **결제 페이지 폼 재제출 확인**(PRG 미적용). 5라운드(41~46번): **장바구니 재담기 시 수량 안 늘어나던 것**, 찜 선택모드 "장바구니 담기" 벌크 버튼 부재, 결제 페이지 포인트 안내 중복/레이아웃 붕괴, **`GET /order/checkout` 405 원인불명 에러**(사용자 직접 재현), 결제 페이지 폭·할인정보 여백 CSS. 6라운드(27번, 47~48번): **상품 상세 리뷰 이미지가 경로 컬럼 누락으로 전부 깨져 있던 것**, **리뷰 좋아요 버튼이 백엔드는 있는데 화면에 아예 없던 것**, 27번(드롭다운 화살표 위치 — `#product-option`/`#coupon_id`가 `appearance:none` 처리 없이 네이티브 화살표만 쓰던 것, 사용자 스크린샷으로 위치 특정 후 해결). 7라운드(49~50번, 2026-09-03 종합 점검): **상품 상세 리뷰 본문의 저장형 XSS**(실증 후 조치), **`/member/userWithdraw` 로그인 가드 누락**. 8라운드(51~53번, 2026-09-03 주석 리팩토링 중): **관리자 마이페이지가 통째로 목업이라 누가 로그인하든 "관리자/admin"으로 보이던 것**, **찜 기본 정렬 "인기순"이 아무 정렬도 안 하던 것**, **중복확인에 서버 "본인 제외"가 없는 것(53번 - 유일한 미해결, JS가 우회 중)**. |
...
| 잠재적 위험 요소 | 28개 — 조치 완료 10 | 20~27번은 원래 "20~27" 그대로 순서가 맞았지만 문서 앞쪽에 잘못 끼어 있던 걸 2026-09-03에 이 레지스트리로 이동만 함(내용 변경 없음). **19·21·22번은 "브라우저가 자동으로 붙이는 상태"(`:visited`/`:focus`/`[hidden]`)를 hover처럼 다루면 안 된다는 같은 계열.** **25·26·27번은 "같은 규칙이 서버·화면 두 곳에 따로 적혀 갈라지는" 계열** — 금액 계산·정책 상수·뷰 이름. 조치 완료지만 재발하기 쉬운 유형이라 새 기능마다 확인할 것. **미조치: 20번(`style_user.css` 최상위 `#id` — 2026-09-03 재실측 108개로 증가)·24번(추가 이미지 표시/검증 불일치)·28번(운영 코드 디버그 출력 42건)** |
...
49. ~~**상품 상세 리뷰 본문에 저장형 XSS(Stored XSS)가 성립함 — 가장 심각**~~ (2026-09-03 종합 점검에서 발견) — `product/productDetail.jsp`가 리뷰 본문을 `${review.reviewText}`로 **이스케이프 없이** 출력하고 있었다. 리뷰 본문은 사용자가 자유롭게 쓰는 값(`VARCHAR2(1500)`)이고, 저장 단계(`ReviewServiceImpl.writeReview`)에도 길이 검사만 있을 뿐 태그 필터가 전혀 없다. 즉 **리뷰에 `<img src=x onerror=...>`나 `<script>`를 쓰면 그 상품 페이지를 여는 모든 사용자의 브라우저에서 실행된다**(자기 화면에서만 터지는 self-XSS가 아니라 저장형이라 심각도가 높다).
    - **실증**: 리뷰 1건의 본문을 `테스트<img src=x onerror=alert(1)>끝`으로 잠시 바꾼 뒤 `/mds/detail/82`를 조회하니 `<p class="review-text">테스트<img src=x onerror=alert(1)>` 로 **태그가 그대로 렌더링**되는 것을 확인(수정 후 재조회하니 `&lt;img ...&gt;`로 이스케이프됨). 테스트에 쓴 리뷰는 원문으로 복구 완료(잔재 0건 확인).
    - 같은 화면의 `${review.nickname}`도 동일하게 노출돼 있었다(닉네임은 정규식 `^[가-힣a-zA-Z0-9_]{2,8}$`로 제한돼 당장은 태그를 못 넣지만, 규칙이 완화되면 바로 같은 문제가 된다).
    - **✅ 조치 완료 (2026-09-03)** — 사용자 입력이 그대로 나오던 곳을 전부 `<c:out>`으로 이스케이프: `productDetail.jsp`(reviewText·nickname·optionName), `order/payment.jsp`(productName·optionName·couponName·addressName·detailAddress), `review/addReview.jsp`(productName·optionName). 점검 스크립트로 재검사해 **남은 미이스케이프 0건** 확인.
    - **앞으로 지킬 것**: JSP에서 사용자·관리자가 입력한 값을 찍을 땐 예외 없이 `<c:out>`. 특히 자유 입력 필드(리뷰 본문, 문의 내용 등)는 반드시. 저장 시 필터링에 기대지 말 것(출력 이스케이프가 정석).

50. ~~**`/member/userWithdraw`에 로그인 가드가 없어 비로그인 상태에서 탈퇴 화면이 열림**~~ (2026-09-03 종합 점검에서 발견) — `MemberController.userWithdraw()`가 세션을 아예 보지 않고 뷰 이름만 반환하고 있었다. 회원 화면 12개를 비로그인으로 전수 호출해보니 **나머지 11개는 전부 302(로그인 리다이렉트)인데 이 화면만 200**이었다.
    - 실제 탈퇴 처리(`POST /member/withdraw`)에는 세션 검사가 있어서 데이터가 지워지지는 않는다(비로그인 POST 시 `"로그인 정보가 없습니다."` 응답 확인). 즉 **화면 노출만의 문제**지만, 로그인하지 않은 사람에게 "회원 탈퇴" 폼이 그대로 보이는 건 혼란을 준다.
    - **✅ 조치 완료 (2026-09-03)** — 같은 클래스의 `deliveryAddressForm()`과 동일한 가드(세션 null이면 flash 메시지 + `redirect:/member/login`) 추가. 수정 후 비로그인 302 / 로그인 200 확인.

51. ~~**관리자 마이페이지의 이름·아이디가 하드코딩되어 누가 로그인하든 "관리자 / admin"으로 보임**~~ (2026-09-03 주석 리팩토링 중 발견) — `admin/adminPage.jsp`가 이름·아이디를 문자열로 박아두고 있었고, 그 위를 `static/js/admin/adminMypageService.js`(목업)가 같은 값으로 다시 덮어쓰고 있었다. 29~33번(카테고리·이동경로·짧은 설명·등급 할인 고정 텍스트)과 **같은 계열**이며, 관리자 계정이 하나뿐이라 눈에 띄지 않았을 뿐이다.
    - 정작 **모델엔 실제 데이터가 이미 있었다** — `MemberController.myPageForm()`이 `loginMember`(실제 `MemberDTO`)를 담아 넘기는데 JSP가 쓰지 않고 있었다.
    - **✅ 조치 완료 (2026-09-03)** — JSP를 `<c:out value="${loginMember.memberName/loginId}"/>`로 교체, `views/adminPage.js`의 목업 렌더링 제거(placeholder 링크 처리만 존치), 이 한 곳에서만 쓰이던 `admin/adminMypageService.js` 삭제.
    - **검증 방법 주의**: `admin` 계정의 실제 이름이 마침 "관리자"라 고쳐도 화면이 똑같아 보인다. 두 번째 관리자 계정(`dummy_admin2` / 이름 `김운영`)을 만들어 `"김운영 (dummy_admin2)"`가 출력되는 것으로 실제 바인딩임을 확인했다(계정은 팀 확인용으로 남겨둠).

52. ~~**찜 목록의 기본 정렬 "인기순"이 아무 정렬도 하지 않음**~~ (2026-09-03 주석 리팩토링 중 발견) — `wish.jsp`의 기본 선택 탭이 `data-sort="popular"`인데 `wishService.sortItems()`에 `popular` 분기 자체가 없어서, 담긴 순서(`WISH_ID desc`)가 그대로 나왔다. "인기순"이라는 라벨만 붙어 있던 셈.
    - 원래 주석은 "실제 인기 지표가 없어서"라고 적혀 있었으나, 확인해보니 **`WishListDTO.reviewCount`가 이미 서버에서 내려오고 있었다**(`wish.xml getWishList`의 `REVIEW_COUNT`).
    - **✅ 조치 완료 (2026-09-03)** — 리뷰 수 내림차순으로 정렬하고, 동점이면 평점이 높은 쪽을 앞에 둔다.

53. ~~**닉네임/이메일/휴대폰 중복확인에 서버 쪽 "본인 제외"가 없음**~~ (2026-09-03 확인 → 같은 날 56번 조치와 함께 해결)
    - `countByNickname`/`countByEmail`/`countByPhone`이 `MEMBER` 전체를 세므로, 회원정보 수정 화면에서 **값을 바꾸지 않고 중복확인을 누르면 자기 자신과 충돌해 "이미 사용중"으로 뜬다.**
    - 현재는 `views/userUpdateInfo.js`가 "현재 저장된 값과 같으면 서버를 호출하지 않고 통과"로 우회하고 있어 실사용엔 드러나지 않는다(저장 성공 때마다 기준값을 갱신).
    - 근본 해결은 **서버 쪽에 본인 제외(`memberId`) 파라미터 추가** — `AND MEMBER_ID != #{memberId}`. 회원가입 경로(memberId 없음)와 수정 경로를 같이 지원해야 하므로 쿼리를 나누거나 `<if>`로 분기하면 된다.
    - 우회가 걷히면(다른 화면에서 같은 API를 쓰거나 JS 기준값이 어긋나면) 바로 드러나므로, 서버에서 막는 쪽이 안전하다.
    - **✅ 조치 완료 (2026-09-03)** — 56번(연락처 중복확인)을 고치자 이 항목이 곧바로 실제 문제가 됐다. 중복확인이 제대로 동작하게 되면서, 회원정보 수정에서 **자기 번호를 다시 타이핑하면 본인 번호인데 "이미 사용중"으로 저장이 막히는** 상태가 된 것(입력창은 타이핑 시 하이픈을 지우는데 프론트 우회 기준값 `currentValues.phone`은 하이픈 포함이라 비교가 어긋난다). 그래서 근본 해결을 같이 넣었다 — 매퍼 3종에 `excludeMemberId`를 받아 `<if>`로 `AND MEMBER_ID != #{excludeMemberId}`를 붙이고, 컨트롤러가 세션의 본인 ID를 넘긴다. 회원가입은 비로그인이라 null이 넘어가 조건이 붙지 않는다. 프론트 우회(`currentValues`)는 그대로 뒀다 - 이제는 서버가 막아주므로 이중 안전장치.


54. ~~**홈/검색결과 카드에서 찜 버튼을 눌러도 옆의 찜 개수가 안 바뀜**~~ (2026-09-03 버그 제보)
    - 하트는 채워지는데(`is-active`) 숫자는 그대로라, 새로고침하기 전까지 화면 값이 서버와 어긋난다. 제보 스크린샷도 하트만 바뀌고 `1` 그대로.
    - 원인은 **화면 코드에만 있었다.** 서버는 정상 — 상품 35 기준 카드 렌더값이 `2 → 3 → 2`로 정확히 따라갔고(`COUNT(DISTINCT W.WISH_ID)`), 헤더 뱃지 소스(`/wish/count`)도 `2 → 3 → 2`. `views/home.js`와 `views/searchProduct.js`가 `btn.classList.toggle('is-active', active)`만 하고 `.product-wish-count-num`을 아무도 안 건드리고 있었다.
    - 같은 계열로 **상품 상세(`views/productdetail.js`)는 반대 결함**이 있었다. 개수는 갱신하지만 **상태가 안 바뀌었을 때도 ±1** 했다 — `toggleWish()`는 비로그인이나 통신 실패면 원래 상태(`wasWished`)를 그대로 돌려주는데, 그걸 "해제됨"으로 읽어 개수를 1 줄였다. 통신 실패는 화면 이동도 없어서 틀린 숫자가 그대로 남는다.
    - **✅ 조치 완료 (2026-09-03)** — `common/cartWishService.js`에 `applyWishState(button, wasWished, nowWished, countEl)`를 두고 세 화면이 공유한다. 하트·`aria-label`·개수를 한 번에 맞추고, **개수는 `nowWished !== wasWished`일 때만** 움직인다. 상품 상세는 개수가 버튼 밖(`#wish-count`)이라 그 요소를 인자로 넘긴다.
    - 찜 목록(`views/wish.js`)은 카드에 개수 표시가 없고 해제 시 카드 제거 후 `location.reload()`라 해당 없음. 헤더 찜/장바구니 뱃지는 `saveWishList → refreshCartBadge → GET /wish/count`로 서버 값을 다시 읽어 와서 정상.

55. ~~**리뷰 좋아요 - 예상 밖 응답을 "취소"로 오해**~~ (2026-09-03, 54번 점검 중 발견)
    - 제보 2번("리뷰 좋아요도 같은 현상인지")을 확인한 결과 **개수 갱신 자체는 정상**이었다. 서버 `5 → 4 → 5`, 화면도 `.review-like-count`를 같이 갱신한다.
    - 다만 `const liked = result === 'on'`이라, 엔드포인트가 500을 내서 **에러 페이지 HTML이 오면 그것도 `'on'`이 아니므로 "취소됨"으로 처리**해 하트가 꺼지고 개수가 1 줄었다. 실제로는 아무 일도 안 일어난 상태다.
    - **✅ 조치 완료 (2026-09-03)** — 응답이 `"on"`/`"off"`가 아니면 화면을 건드리지 않고 실패 안내만 띄운다.

56. ~~**연락처 중복확인이 어떤 번호든 항상 "사용 가능"으로 나옴**~~ (2026-09-03 버그 제보)
    - 제보: "010-9999-9999로 가입한 유저가 로그인 후 로그아웃 했을 때 휴대폰 중복 체크에 여전히 사용 가능한 번호로 체크됨"
    - **저장 형식과 조회 형식이 달랐다.** `signUp`은 `normalizePhone()`으로 `010-9999-9999`(하이픈)로 저장하는데, 중복확인 API는 입력값을 그대로 `WHERE PHONE = #{phone}`에 넣었다. 가입 폼은 숫자만 받으므로(`MemberDTO`의 `^01[0-9]{8,9}$`) 넘어가는 값은 늘 `01099999999` — **하이픈 형식과 절대 안 맞아서 특정 번호가 아니라 모든 번호가 "사용 가능"이었다.** 로그아웃 여부와는 무관.
    - 실측: `01099999999` → "사용 가능" / `010-9999-9999` → "이미 사용중". DB는 28명 중 25명이 하이픈형(나머지 3명은 탈퇴 마스킹 `'9' || MEMBER_ID`).
    - DB는 안전했다 — `UK_MEMBER_PHONE` UNIQUE 제약이 있고, `signUp`은 정규화 **후** `isPhoneCheck`를 부르므로 실제 중복 가입은 막혔다. **중복확인 버튼만 거짓말을 하고 있었고**, 그대로 제출하면 그때서야 막혔다.
    - **✅ 조치 완료 (2026-09-03)** — `normalizePhone`(저장용, 형식 아니면 예외)에서 `formatPhone`(조회용, 형식 아니면 null)을 분리하고 `isPhoneCheck`가 조회값도 저장 형식으로 맞추게 했다. 숫자만 먼저 걸러내므로 하이픈이 붙은 값을 다시 넣어도 결과가 같다(멱등) — `signUp`/`phoneUpdate`가 정규화 후 다시 호출해도 안전.
    - 검증: `01099999999` / `010-9999-9999` / `010 9999 9999` / `0109999-9999` 네 형태 모두 "이미 사용중", 미사용 번호는 "사용 가능", 10자리(`011-987-6543`)도 동일.


57. ~~**결제 화면에서 배송지를 추가하면 회원정보 수정 화면으로 튕김 (TC-35)**~~ (2026-09-03 QA 제보)
    - `MemberController.addAddress`가 어디서 들어왔든 **무조건 `redirect:/member/updateInfo`** 였다. 입력·저장 자체는 정상이라 "실패"로 잡힌 건 복귀 경로뿐.
    - `/member/deliveryAddress`(독립 배송지 추가 화면)로 들어오는 링크는 **결제 화면 한 곳뿐**이다(회원정보 수정 화면은 자체 인라인 배송지 UI를 따로 쓴다). 즉 이 리다이렉트는 사실상 모든 경우에 틀린 목적지였다.
    - **✅ 조치 완료 (2026-09-03)** — `returnUrl`을 결제 화면 → 배송지 화면(hidden input) → `addAddress`로 전달하고, 기존 `isSafeRedirect`(우리 사이트 내부 경로만 허용)를 통과할 때만 그쪽으로 보낸다. 없거나 외부 주소면 종전대로 `/member/updateInfo`.
    - 검증: `returnUrl=/order/payment` → 결제 화면 복귀 / 미지정 → 회원정보 수정 / `https://evil.com`·`//evil.com` → 차단되어 회원정보 수정. 기본배송지(`isDefault=Y`)로 추가하면 돌아온 결제 화면에 새 주소가 반영된다(`paymentResume`가 매번 `cartPrepare`로 다시 조회하므로 세션 스냅샷 문제 없음). 비로그인 가드도 유지.

58. ~~**장바구니 상품 이미지가 썸네일 틀을 벗어남 (TC-36)**~~ (2026-09-03 QA 제보)
    - `.cart-container .item-thumbnail`은 70×70 고정인데 **내부 `img`에 대한 규칙이 아예 없었다.** 그래서 `default.css`의 `img { max-width:100%; height:auto }`가 그대로 먹어, 세로로 긴 사진이 폭 70px에 맞춰지면서 높이는 원본 비율대로 늘어나 틀 아래로 흘러넘쳤다. `overflow:hidden`도 없어 `border-radius`가 잘라주지도 못했다.
    - 실측: 조니워커(상품 35) 594×896 → **35px 초과**, Dewars(상품 46) 259×319 → 16px 초과, 인사이디(상품 115) 860×1494 → 51px 초과. 제보 스크린샷의 상품이 정확히 이 목록에 있다. 정사각(750×750) 상품만 담긴 장바구니에서는 재현되지 않는다.
    - **✅ 조치 완료 (2026-09-03)** — 틀에 `overflow:hidden`, 내부 `img`에 `width/height:100% + object-fit:cover`를 넣었다. 프로젝트에 이미 같은 패턴이 세 곳(리뷰 작성 `.product-thumbnail`, 주문/배송 `.item-thumb`·`.order-item-thumb`, 홈/검색 `.product-img`) 있었고 **장바구니만 빠져 있었다.** "이미지 없음" 문구도 70px 안에 들어가도록 같이 정리.
    - 전수 점검: 고정 px 크기 틀 70개를 훑어 `<img>`가 실제로 들어가는 컨테이너를 추렸고, 장바구니 외에는 전부 `img` 규칙이 있었다.


59. ~~**결제 화면 - 배송지 등록/서버 검증 실패로 되돌아오면 포인트·쿠폰·결제수단 선택이 초기화됨 (TC-39)**~~ (2026-09-03 QA 제보)
    - 이용약관 미체크는 코드를 추적해보니 `payment.js`가 `event.preventDefault()`만 하고 페이지 이동이 전혀 없어 애초에 상태가 지워질 수 없다(이 경로는 재현 안 됨).
    - 실제 원인은 **화면이 완전히 새로 그려지는 두 경로**다. 포인트(`#used_point`)·쿠폰(`#coupon_id`)·결제수단(`input[name="paymentId"]`)은 전부 서버에 저장되는 값이 아니라 이 화면의 입력 상태일 뿐인데:
      1. "+배송지 추가"로 나갔다가 돌아옴(`GET /order/payment` → `paymentResume()`가 fresh 렌더)
      2. 결제하기를 눌렀는데 서버 쪽 검증(재고/쿠폰 등)에 걸림(`OrderController.checkout`의 `catch` → `redirect:/order/payment`)
    - 둘 다 서버가 매번 기본값(포인트 0, 쿠폰 미선택, 결제수단 미선택)으로 새로 그리므로 입력이 전부 사라진다.
    - **✅ 조치 완료 (2026-09-03)** — `sessionStorage`로 선택값을 임시 저장·복원한다. 포인트/쿠폰/결제수단 변경 시, "+배송지 추가" 클릭 직전, 결제 제출 시도 시점에 저장하고, 페이지 로드 시 있으면 복원 후 1회성으로 지운다. 저장된 쿠폰이 그사이 소진/만료돼 선택지에 없어지면 조용히 무시(빈 값으로).
    - 검증: 가짜 DOM/sessionStorage로 정상 왕복·1회성 소거·draft 없을 때 무변경·소멸된 쿠폰 무시·저장 실패 시 예외 안 던짐 5개 시나리오 전부 통과. 실서버로 배송지 버튼 `data-href` 렌더 확인.

60. ~~**상품 상세 페이지에 쿠폰 발급 버튼이 없음 (TC-41)**~~ (2026-09-03 QA 제보)
    - `ProductController.getCoupon`(`GET /mds/coupon/{couponId}`)이 있긴 했지만 **뷰 어디에서도 호출하지 않는 죽은 코드**였다(3-57 시점부터 알려져 있던 상태). 단일 쿠폰 ID를 받아 `home/home`으로 리다이렉트하는 구조라 AJAX 버튼에 쓸 수도 없었고, `System.out.println`도 있었다(AUDIT 28 계열).
    - COUPON 테이블엔 PRODUCT_ID가 없다 - 쿠폰은 상품별이 아니라 **매장 전체 쿠폰**이다. 그래서 요구사항("버튼 클릭 시 발급 가능한 모든 쿠폰 발급")대로 **한 번 클릭에 발급 가능한 쿠폰을 전부 발급**하는 방식으로 다시 만들었다.
    - **✅ 조치 완료 (2026-09-03)**:
      - `ProductServiceImpl.issueAllCoupons(memberId)` 신설 - 발급 가능 쿠폰을 순회하며 미발급인 것만 `insertCoupon`, 새로 발급된 개수 반환. `@Transactional`.
      - `getCoupons` 쿼리(`detailPage.xml`)에 **`WHERE DEADLINE >= TRUNC(SYSDATE)` 필터를 새로 추가** - 원래 만료된 쿠폰까지 전부 내려주고 있었다(마이페이지 쿠폰함과 동일 기준으로 맞춤). 이게 없으면 지난 쿠폰까지 발급해 버리는 문제였다.
      - `couponCheck`(기존 쿼리, 손 안 댐)는 TYPE 무관하게 이력 존재 여부만 보므로 "쿠폰마다 한 번씩만"을 정확히 만족한다.
      - `ProductController`: `GET /mds/coupon/{couponId}` → `POST /mds/coupon/issue`(`@ResponseBody`, `ApiResponse<Integer>`)로 교체. 비로그인은 `ApiResponse.fail`.
      - `productDetail.jsp`에 "쿠폰 받기" 버튼, `productdetail.js`에 클릭 핸들러(발급 결과 메시지 alert, 비로그인이면 로그인 화면 이동).
    - 검증(`dummy_sgok`, 쿠폰 이력 0건인 계정): 비로그인 호출 → 로그인 필요 메시지 / 첫 호출 → "6개의 쿠폰이 발급되었습니다"(현재 유효 쿠폰 전량) / 바로 재호출 → "받을 수 있는 새 쿠폰이 없습니다"(0개, 중복 발급 안 됨) / GET으로 접근 시 405 / **마이페이지 쿠폰함에 6건 전부 노출** / **결제 화면 쿠폰 선택 목록에 6건 전부 노출**(select value 25·26·27·28·29·30). `mvnw compile` EXIT 0, 스모크 39개 지표 차이 없음.
    - `dummy_sgok`는 이제 쿠폰 6종 보유 + 장바구니 1건(popId 125) 상태라, 브라우저에서 "쿠폰 받기" 버튼 클릭 → 결제 화면에서 선택 → 실제 결제까지 한 번에 훑어보기 좋은 계정이다.
    - **후속 (2026-09-03, 3-69)**: 버튼 위치를 이미지 갤러리 아래 독립 배너로 옮기고, 클릭 즉시 전체 발급 대신 발급 가능 목록(쿠폰명+할인율)을 모달로 먼저 보여준 뒤 "모두 받기"에서 발급하도록 개편. `GET /mds/coupon/issuable` 신설. 자세한 내용은 HANDOFF 3-69.
...
20. **`style_user.css`에 스코프 없는 최상위 `#id` 선택자가 108개 남아 있음** (2026-09-03 재실측 - 2026-09-01의 102개에서 늘어남. 그 전엔 "약 90개"로 적혀 있었다) — `header.jsp`가 CSS 4개를 전 페이지에 무조건 로드하는 구조라, 잠재적으로 3-5/3-6과 같은 계열의 전역 leak 위험. 특히 **`#title`은 서로 다른 JSP 10개가 쓰고 있는 매우 일반적인 id**다(`addCoupon`/`admincouponView`/`login`/`signUp`/`usercouponView`/`userUpdateInfo`/`userWithdraw`/`orderComplete`/`payment`/`deliveryAddress`). 현재는 대부분 `.login-card #title`처럼 페이지별로 한 단계 더 스코프한 규칙이 덮고 있어 문제가 안 보이지만, **최상위 `#title{}`과 `#title h1`/`#title p`는 여전히 스코프 규칙이 없는 페이지로 새어 나간다.** `#buttonArea`/`#cancel-button`/`#add-button`/`#product-info` 같은 범용 id도 같은 위험.
...
28. **운영 코드에 디버그 출력(`System.out.print`)이 42건 남아 있음** (2026-09-03 종합 점검에서 발견, **미조치**)
    - 9개 파일에 분포: `CartController`(8) / `OrderController`(7) / `MemberController`(6) / `WishController`(6) / `OrderServiceImpl`(5) / `ProductServiceImpl`(4) / `CartServiceImpl`(3) / `ProductController`(2) / `WishServiceImpl`(1).
    - 그중 11건은 주석에 "추적용", "로그인 체크용 나중에 삭제"라고 명시돼 있어 **원래 지울 예정이었던 것**들이다.
    - 문제: (1) 세션 객체를 통째로 찍는 곳이 있어 **회원 정보가 서버 콘솔 로그에 남는다**(`System.out.println("세션 저장 완료: " + session.getAttribute(...))`), (2) 주문/배송 목록을 통째로 출력하는 곳은 데이터가 늘수록 콘솔이 느려진다.
    - 지금 당장 기능을 깨뜨리진 않아 이번엔 손대지 않았다. 여러 담당자 파일에 걸쳐 있어 한 번에 지우면 병합 충돌이 커지므로, **각 영역을 만질 때 함께 정리하거나 팀 합의 후 일괄 정리**하는 게 안전하다.
