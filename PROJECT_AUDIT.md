...
| 섹션 | 항목 수 | 비고 |
|---|---|---|
| 버그 (기능이 실제로 깨져 있음) | 18개 — 조치 완료 4 · 부분 조치 3 · 미해결 11 | 미해결분 대부분이 product/review/member 담당 영역. **17번(다품목 주문의 리뷰 경로 상실)·18번(빠른메뉴 리뷰 타일 무반응) 조치 완료.** 다만 다품목 주문의 **품목별 리뷰 작성 UI**는 아직 없음(HANDOFF 3-54-9에 설계만) |
