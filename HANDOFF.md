...
**이 문서를 읽는 법**
- `HANDOFF.md`(작업 로그) / `PROJECT_AUDIT.md`(버그·정책·위험 목록) / `HANDOFF_NEXT_SESSION_PROMPT.txt`(다음 세션 시작 프롬프트) 3개 다 **git commit 대상이 아님** — 사용자가 직접 관리하는 문서이고, 커밋도 항상 사용자가 직접 한다(Claude가 먼저 커밋하지 않는 것이 이 프로젝트의 규칙).
...
## 지금 상태 요약 (2026-09-01 기준)

| 항목 | 상태 |
|---|---|
| 현재 브랜치 | `frontfix` (HEAD `e30a369` — `JWC_works`에 `Util_branch`를 병합해온 지점, **로컬 전용 임시 브랜치**) |
| 브랜치 성격 | `frontfix`는 프론트 담당자(JWC1226)가 못 끝낸 CSS/JS 규격화를 **"여기까지 끝냈으니 나머지는 이 기준대로"** 하고 넘겨주려고 만든 **전달용 임시 브랜치**. 장기 기준선 아님 |
| 실제 작업 브랜치 | **`BJY_works`** — wish/cart/쿠폰 패키지 정리(3-34/3-35/3-36)가 전부 반영된, 서버 쪽이 가장 정리된 상태. `frontfix`엔 그 정리분이 안 들어와 있음(브랜치 시차이지 버그 아님 — `frontfix`에서 다시 고치지 말 것) |
| 미커밋 변경 | `mappers/member/MemberMapper.xml`, `css/style.css`, `css/style_user.css`, `home/home.jsp` + 신규 `js/product/homeProductService.js`, `js/views/home.js` |
| 진행 중인 작업 | **프론트엔드 CSS/JS 규격화** — 홈페이지(`home.jsp`)를 기준 페이지로 삼아 패턴을 확립하는 단계까지 완료 |
| 다음 단계 | 홈페이지 결과물을 프론트 담당자에게 전달 → 병합 → 나머지 페이지에 같은 패턴(CSS 통합 + JS 분리) 적용. **서버 연동은 범위 밖** — 상세 계획은 아래 `3-41` |
...
| `3-41` | 2026-09-01 | 문서 정리 + **다음 단계(다른 페이지 규격화) 계획** ← 다음 세션은 여기서 시작 |
